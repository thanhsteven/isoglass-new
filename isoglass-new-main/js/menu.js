	...
	navMenu.addEventListener("click", (event) => {
		if (event.target.hasAttribute("data-toggle") && window.innerWidth <= mediaSize) {
			...
			if (menuItemHasChildren.classList.contains("menu-item-active")) {
				...
				const subMenuSimple = menuItemHasChildren.querySelector(".sub-menu-simple");
				subMenuSimple.style.maxHeight = subMenuSimple.scrollHeight + "px";
			}
		}

		function collaseSubMenu() {
			navMenu.querySelector(".menu-item-has-children.menu-item-active").classList.remove("menu-item-active");
			navMenu.querySelector(".menu-item-has-children.menu-item-active .sub-menu-simple").removeAttribute("style");
		}
	});
